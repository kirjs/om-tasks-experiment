...
(defn do-tick [app]
  (if (:running (:state @app))
    (do (om/transact! app [:state :timeUntilTheNextTask] dec)
        (if (< (:timeUntilTheNextTask (:state @app)) 0) (go-next app)))))



(defn set-up-timer [app] (start-timer-callback #(do-tick app)))
(defn indicator-progress [time left] (str (* 100 (- 1 (/ left time))) "%"))

(defn progress-indicator [task app]
  (dom/div #js {:className "indicator"}
           (dom/div #js
...
(defn single-task-view
  [{task :task index :index active :active app :app}]
  (reify
    om/IRender
    (render [_]
      (dom/li #js {:className (if (= index active) "active")}
              (dom/div #js {:className "task"}
                       (dom/div #js {:className "time"} (format-time (:time task)))
                       (dom/div #js {:className "name"} (:name task))
                       (dom/img #js {
                                     :onClick   #(activate-task-by-index app index)
                                     :className "play"
                                     :src       "data:image/png;base64,"})
                       ))
      )))
...
(defn prepare-state [state] state)
